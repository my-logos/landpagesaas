...
document.addEventListener('DOMContentLoaded', function () {
    const productImagesDataElement = document.getElementById('product-images-data');
    if (productImagesDataElement) {
        try {
            const imagesData = JSON.parse(productImagesDataElement.getAttribute('data-images'));
            if (imagesData && Array.isArray(imagesData) && imagesData.length > 0) {
                initializeProductImages(imagesData);
            }
        } catch (e) {
