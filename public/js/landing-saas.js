document.addEventListener('DOMContentLoaded', function () {
  ...
  // Set variation color box background from data-color attribute
  document.querySelectorAll('.variation-color-box[data-color]').forEach(function (el) {
  ...
  // Smart Coupon Banner - Show after 3 seconds
  var smartCouponBanner = document.getElementById('smart-coupon-banner');
  if (smartCouponBanner) {
    setTimeout(function () {
      smartCouponBanner.classList.remove('hidden');
      smartCouponBanner.style.display = 'block';
    }, 3000);
  }

  // Smart Coupon Banner - Close button
  var couponCloseBtn = document.querySelector('.coupon-close');
  if (couponCloseBtn) {
  ...
  // Set fixed order button background color from data attribute
  document.querySelectorAll('.fixed-order-button[data-button-color]').forEach(function (el) {
  ...
  document.querySelectorAll('.hero-background-image[data-bg-image]').forEach(function (el) {
    var bgImage = el.getAttribute('data-bg-image');
    if (bgImage && bgImage.trim() !== '') {
      el.style.backgroundImage = 'url(' + bgImage.trim() + ')';
    }
  });

  // Handle image error - hide parent element
