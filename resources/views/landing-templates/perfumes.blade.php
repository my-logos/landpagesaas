@php
$templateCss = 'css/landing-perfumes.css';
$additionalFonts = '
...
<body data-google-analytics-id="{{ $page->google_analytics_id ?? '' }}"
    ...
    <div class="top-banner">
        {{ $page->additional_description ?? 'عرض حصري على العطور! احصل على خصم فوري، العرض محدود!' }}
    </div>

    <!-- Hero Section - Luxury Premium Design -->
    <section class="hero-section hero-luxury-premium">
        <div class="hero-luxury-overlay"></div>
        <div class="container">
            <div class="hero-luxury-content">
                <div class="hero-luxury-text">
                    <div class="hero-luxury-badge">
                        <i class="fa-solid fa-spray-can"></i>
                        <span>عطور فاخرة</span>
                    </div>
                    <h1 class="hero-title-luxury">{{ $page->title ?? ($page->product->name ?? 'عطور فاخرة') }}</h1>
                    <p class="hero-description-luxury">
                        {{ $page->content ?? ($page->product->description ?? 'اكتشف مجموعتنا المميزة من العطور الفاخرة. روائح مميزة وجودة عالية.') }}
                    </p>
                    @if($page->product)
                    <div class="pricing-luxury">
                        <div class="price-luxury-wrapper">
                            <span class="current-price-luxury">EGP {{ number_format($finalPrice, 2) }}</span>
                            <span class="old-price-luxury">EGP {{ number_format($smartCouponsEnabled ? $increasedPrice : $originalPrice, 2) }}</span>
                        </div>
                        <span class="discount-luxury">خصم {{ round((($page->product->price_cents - ($page->product->price_cents * 0.75)) / $page->product->price_cents) * 100) }}%</span>
                    </div>
                    @endif
                    ...
                    <a href="#order-form" class="btn-luxury-cta">
                        <i class="fa-solid fa-gem"></i>
                        <span>احصل على عطرك الفاخر</span>
                    </a>
                </div>
                <div class="hero-luxury-visual">
                    @if($page->product && $page->product->images)
                    @if($mainImage)
                    <div class="hero-luxury-image-container">
                        <div class="luxury-frame"></div>
                        <img src="{{ $mainImage }}" alt="{{ $page->product->name ?? 'Product' }}" class="hero-luxury-image">
                    </div>
                    @else
                    <div class="hero-luxury-placeholder">
                        <i class="fa-solid fa-spray-can"></i>
                    </div>
                    @endif
                    @else
    ...
    <section class="features-section">
        <div class="container">
            <h2 class="section-title">لماذا تختار عطورنا؟</h2>
            <div class="features-grid">
                ...
                <div class="feature-card">
                    ...
                    <h3 class="feature-title">روائح مميزة</h3>
                    <p class="feature-description">روائح فاخرة من أفضل العلامات التجارية</p>
                </div>
                <div class="feature-card">
                    <div class="feature-icon">
