...
<head>
    ...
    <title>{{ app()->getLocale() === 'ar' ? 'تذكرة دعم جديدة' : 'New Support Ticket' }}</title>
</head>
                ...
                <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 8px; overflow: hidden;">
                    <tr>
                        <td style="padding: 30px; background-color: #1f2b6b;">
                            <h1 style="margin: 0; color: #ffffff; font-size: 24px;">{{ app()->getLocale() === 'ar' ? 'تذكرة دعم جديدة' : 'New Support Ticket' }}</h1>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 30px;">
                            <h2 style="margin: 0 0 20px 0; color: #333333; font-size: 20px;">{{ app()->getLocale() === 'ar' ? 'تم إنشاء تذكرة دعم جديدة' : 'A new support ticket has been created' }}</h2>
                            <p style="margin: 0 0 15px 0; color: #666666; font-size: 16px; line-height: 1.6;">
                                {{ app()->getLocale() === 'ar' ? 'تم إنشاء تذكرة دعم جديدة من قبل مستخدم.' : 'A new support ticket has been created by a user.' }}
                            </p>
                            ...
                            <table width="100%" cellpadding="0" cellspacing="0" style="margin: 20px 0; background-color: #f8f9fa; border-radius: 5px;">
                                <tr>
                                    <td style="padding: 15px;">
                                        ...
                                        <p style="margin: 8px 0; color: #333333; font-size: 14px;"><strong>{{ app()->getLocale() === 'ar' ? 'الأولوية:' : 'Priority:' }}</strong> {{ ucfirst($ticket->priority) }}</p>
                                        <p style="margin: 8px 0; color: #333333; font-size: 14px;"><strong>{{ app()->getLocale() === 'ar' ? 'التاريخ:' : 'Date:' }}</strong> {{ $ticket->created_at->format('Y-m-d H:i') }}</p>
                                        <p style="margin: 8px 0; color: #333333; font-size: 14px;"><strong>{{ app()->getLocale() === 'ar' ? 'الرسالة:' : 'Message:' }}</strong></p>
                                        <p style="margin: 8px 0; color: #666666; font-size: 14px; line-height: 1.6; white-space: pre-wrap;">{{ $ticket->message }}</p>
                                    </td>
                                </tr>
                            </table>
