...
<div dir="{{ $dir }}">
  <div class="success-container">
    ...
    <h1>{{ $t('messages.payment_successful') ?? 'Payment Successful' }}</h1>
    ...
    <div class="order-info">
      ...
      <div class="info-row">
        <span class="info-label">{{ $t('messages.invoice_number') ?? 'Invoice Number' }}:</span>
        <span class="info-value">{{ $invoice->invoice_number }}</span>
      </div>
      <div class="info-row">
        <span class="info-label">{{ $t('messages.amount') ?? 'Amount' }}:</span>
        <span class="info-value">{{ number_format($invoice->amount, 2) }} {{ $invoice->currency }}</span>
      </div>
      <div class="info-row">
        <span class="info-label">{{ $t('messages.payment_method') ?? 'Payment Method' }}:</span>
        <span class="info-value">{{ ucfirst($invoice->payment_method) }}</span>
      </div>
      <div class="info-row">
        <span class="info-label">{{ $t('messages.date') ?? 'Date' }}:</span>
